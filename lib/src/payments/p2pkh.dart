...
class P2PKH {
  ...
  void _getDataFromHash() {
    if (data.address == null && network?.pubKeyHash != null && data.hash != null) {
...
class P2PKHData extends PaymentData {
  P2PKHData({address, hash, output, pubkey, input, signature, witness})
      : super(address: address, hash: hash, output: output, pubkey: pubkey, input: input, signature: signature, witness: witness);
}
